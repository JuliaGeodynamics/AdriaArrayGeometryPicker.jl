...
function start_AdA_Picker(;data=nothing)
    ...
    logo_img = load(joinpath(@__DIR__,"../assets/AdA_Picker_logo_tr.png")) # load the logo
    ...
    on(menu_fileIO.selection) do s
        if s == "Load Profile..."
            ...
            @async begin 
                ...
                text!(topo_ax1,0.5*(maximum(x_topo)-minimum(x_topo)),maximum(y_topo);text = "profilename", align = (:center, :center),offset = (0, 20))
        ...
        elseif s ==  "Save Picks..."
            ...
            # interpolate the lat and lon of the picked points from the profile start and end points
            # this is done by creating a linear interpolation function for the lat and lon from the profile
            ...
            # create a vector for each pick with x, depth, lat and lon
            # then merge everything a a named tuple
            x_pick = [picks[][ipick][1] for ipick in 1:length(picks[])]
            y_pick = [picks[][ipick][2] for ipick in 1:length(picks[])]
